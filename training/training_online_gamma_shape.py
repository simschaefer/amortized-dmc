...
from dmc import DMCgamma

num_batches_per_epoch = 250

#########
network_name = "dmc_sdr_estimated_4afc_gamma_settrans_" + str(epochs) + '_' + slurm_id
#########

print(network_name, flush=True)

model_specs = {"simulation_settings": {
                                       # prior_means/prior_sds for 'a' (index 1) are unused: a's prior is
                                       # overridden by a_gamma_shape/a_gamma_scale below (a = 1 + Gamma(2.0, 1.5)).
                                       "prior_means": np.array([32.40, 2.0, 0.49, 387.53, 48.25, 88.21]),
                                       "prior_sds": np.array([9.05, 0.5, 0.15, 57.65, 10.41, 16.56]),
                                       "tmax": 2000,
                                       "contamination_probability": None,
                                       "time_to_peak": 93.88,
                                       "a_gamma_shape": 2.0,
                                       "a_gamma_scale": 1.5,
                                       'param_lower_bound' : np.array([0, 1.5, 0, 0, 0, 0]),
                                       "fixed_num_obs": None,
                                       'param_names': ("A", "a", "mu_c", "mu_r", "sd_r", "b")},
...
simulator = DMCgamma(**model_specs['simulation_settings'])
