...
class DMCasym:
    def __init__(
        ...
        """
        Initialize the DMC simulator (asymmetric amplitude variant) in a BayesFlow-friendly format.

        Identical to the standard DMC simulator, except that the automatic-activation
        amplitude magnitude is estimated separately per condition via `A_con`
        (congruent) and `A_inc` (incongruent), instead of both conditions sharing
        a single `A`. Both are estimated as positive magnitudes with the same
        prior/truncation, and `A_inc` is negated internally in `experiment()` so
        that the incongruent condition's automatic activation still opposes the
        congruent condition's, as in the standard DMC's sign-flipped `A`.
        ...
        param_names : tuple of str, optional
            Names of the parameters. Default is ('A_con', 'A_inc', 'tau', 'mu_c', 'mu_r', 'b', 'sd_r').
        ...
        allowed_names = {'A_con', 'A_inc', 'tau', 'mu_c', 'mu_r', 'b', 'sd_r'}
        ...
        if self.sdr_fixed is None:
            required_names = {'A_con', 'A_inc', 'tau', 'mu_c', 'mu_r', 'b', 'sd_r'}
        else:
            required_names = {'A_con', 'A_inc', 'tau', 'mu_c', 'mu_r', 'b'}
    ...
    def prior(self, rng: np.random.Generator | None = None) -> dict[str, float]:
        ...
        >>> params
        {'A_con': 1.23, 'A_inc': 0.87, 'tau': 45.6, 'mu_c': 0.12, 'mu_r': 210.3, 'b': 90.1, 'sd_r': 15.2}
        """
    ...
    def trial(self,
        ...
        ----------
        A : float
            Amplitude of the time-dependent control signal for this condition
            (either `A_con` or `-A_inc`, passed in by `experiment()`).
        tau : float
    ...
    def experiment(
        ...
        ----------
        A_con : float
            Amplitude of the control signal for congruent trials.
        A_inc : float
            Magnitude of the control signal for incongruent trials, estimated
            as a positive value (same prior as `A_con`). Internally negated
            before being passed to `trial()`, so that its automatic activation
            opposes `A_con`'s.
        tau : float
            Time constant governing the shape of the control signal.
        mu_c : float
        ...
        # simulate CONGRUENT trials (amplitude A_con)
        data[:obs_per_condition] = self.trial(
            A=A_con, tau=tau, mu_c=mu_c, b=b, t=t, noise=noise[:obs_per_condition], non_decision_ts=non_decision_ts[:obs_per_condition], rng=rng
        )

        # simulate INCONGRUENT trials (amplitude -A_inc: A_inc is estimated as a positive
        # magnitude, same as A_con, and negated here so its automatic activation opposes
        # the congruent condition's, as in the standard DMC's sign-flipped A)
        data[obs_per_condition:] = self.trial(
            A=-A_inc, tau=tau, mu_c=mu_c, b=b, t=t, noise=noise[obs_per_condition:], non_decision_ts=non_decision_ts[obs_per_condition:], rng=rng,
        )
    ...
    def sample(self,
        ...
        dict[str, np.ndarray]
            ...
            - parameter names (e.g., "A_con", "A_inc", "tau", ...)
            - "rt" : reaction times
    ...
    def __call__(self,
        ...
        **kwargs
        ...
        dict[str, np.ndarray | float | int]
            ...
            - parameter names (e.g., "A_con", "A_inc", "tau", "mu_c", "mu_r", "b", "sd_r")
            - "rt" : np.ndarray, shape (num_obs,)
