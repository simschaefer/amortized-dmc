...
class DMCfull:
    def __init__(
        ...
        param_names: tuple[str] = ('A', 'a', 'tau', 'mu_c', 'mu_r', 'b', 'sd_r'),
        param_lower_bound: tuple[float] = (0, 1.5, 0, 0, 0, 0, 0),
        ...
        param_names : tuple of str, optional
            Names of the parameters. Default is ('A', 'a', 'mu_c', 'mu_r', 'b', 'sd_r').
        param_lower_bound : tuple of floats
            Lower bound for the prior. Note that a <= 1 will break the simulations.
        ...
        a_gamma_shape : float or None, optional
            ...
            shifted Gamma distribution, `a = param_lower_bound[a] + Gamma(a_gamma_shape,
        ...
        allowed_names = {'A', 'a', 'tau', 'mu_c', 'mu_r', 'b', 'sd_r'}
        ...
        if self.sdr_fixed is None:
            required_names = {'A', 'a', 'tau', 'mu_c', 'mu_r', 'b', 'sd_r'}
        else:
            required_names = {'A', 'a', 'tau', 'mu_c', 'mu_r', 'b'}
        ...
        if self.a_gamma_shape is not None:
            if self.a_gamma_shape <= 1:
                raise ValueError(
                                    ...
                                    "density diverges at its lower bound instead of tapering to zero there."
                                )
            if self.a_gamma_scale <= 0:
                raise ValueError(f"a_gamma_scale = {self.a_gamma_scale}. Must be > 0.")

    def derived_quantities(self, a, tau) -> dict[str, np.ndarray | float]:
        """
        Compute derived DMC timing quantities from the shape and scale of the
        automatic-activation gamma pulse.

        Parameters
        ----------
        a : float or np.ndarray
            Gamma shape parameter(s). Must be > 1.
        tau : float or np.ndarray
            Gamma scale / characteristic-time parameter(s). Must be > 0.

        Returns
        -------
        dict[str, float or np.ndarray]
            "t_max": peak latency, tau * (a - 1).
            "t_90": latency at which 90% of the gamma cumulative mass has
                been emitted, i.e. gamma.ppf(0.9, a, scale=tau).
        """
        t_max = tau * (a - 1)
        t_90 = gamma.ppf(0.9, a, scale=tau)
        return {"t_max": t_max, "t_90": t_90}
    ...
    def prior(self, rng: np.random.Generator | None = None) -> dict[str, float]:
        ...
        >>> params
        {'A': 1.23, 'a': 3.4, 'mu_c': 0.12, 'mu_r': 210.3, 'b': 90.1, 'sd_r': 15.2}
        """
        ...
        params = dict(zip(self.param_names, p))
        derived = self.derived_quantities(params['a'], params['tau'])
    ...
    def trial(self, 
        ...
        a : float
            Shape of the reshaped gamma distribution (automatic activation)
        mu_c : float
    ...
    def experiment(
        ...
        **kwargs
        ...
        a : float
            Gamma shape of the automatic activation. 
        mu_c : float
        ...
        data[:obs_per_condition] = self.trial(
            A=A, tau=tau, a=a, mu_c=mu_c, b=b, t=t, noise=noise[:obs_per_condition], non_decision_ts=non_decision_ts[:obs_per_condition], rng=rng
        )
    ...
    def __call__(self, 
        ...
        **kwargs
        ...
        dict[str, np.ndarray | float | int]
            ...
            - parameter names (e.g., "A", "a", "mu_c", "mu_r", "b", "sd_r")
            - "rt" : np.ndarray, shape (num_obs,)
        ...
        # compute t at maximum and t 90th:
        derived = self.derived_quantities(prior_draws['a'], prior_draws['tau'])

        return prior_draws | sims | derived
