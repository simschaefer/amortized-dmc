...
class DMCsoa:
    def __init__(
        ...
        """
        Initialize the DMC simulator (with stimulus onset asynchrony, SOA) in a BayesFlow-friendly format.

        This is identical to the plain `DMC` simulator, except that the automatic
        (distractor-driven) activation starts at t=0 while the controlled
        (target-driven, `mu_c`) activation only switches on `soa` milliseconds
        later - modeling flanker/distractor onset preceding target onset.
    ...
    def trial(self,
        ...
        """
        Simulate multiple DMC trials in parallel, with the controlled activation
        delayed by `self.soa` milliseconds relative to the automatic activation.
        ...
        The automatic (distractor-driven) component of the drift is active from
        t=0. The controlled (target-driven) component, `mu_c`, is held at zero
        until t=`self.soa` and switched on from then onward - modeling a
        distractor/flanker that appears `self.soa` ms before the target.
        ...
        mu_c : float
            Constant (baseline) drift component, active only for t >= self.soa.
        b : float
        ...
        # Controlled activation switches on soa ms after the automatic activation (t=0)
        soa_idx = np.searchsorted(t, self.soa, side='left')
        mu_c_vec = np.zeros_like(t)
        mu_c_vec[soa_idx:] = mu_c
        ...
        if self.a_value != 2:
            ...
            mu_t = A * exponent_term * power_term * deriv_term + mu_c_vec  # shape (T,)

        else:
            mu_t = A/tau * np.exp(1 - t/tau) * (1 - t/tau) + mu_c_vec
