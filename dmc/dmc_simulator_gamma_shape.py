...
class DMCgamma:
    def __init__(
        ...
        param_names: tuple[str] = ('A', 'a', 'mu_c', 'mu_r', 'b', 'sd_r'),
        time_to_peak: float = 100,
        param_lower_bound: tuple[float] = (0, 1.5, 0, 0, 0, 0),
        ...
        time_to_peak : float
            Defines the fixed time in ms until the maximum amplitude of automatic activation (A) is reached.
        ...
        a_value : int, optional
            Unused by the simulation itself (the gamma shape actually used in
        ...
        allowed_names = {'A', 'a', 'mu_c', 'mu_r', 'b', 'sd_r'}
        ...
        if self.sdr_fixed is None:
            required_names = {'A', 'a', 'mu_c', 'mu_r', 'b', 'sd_r'}
        else:
            required_names = {'A', 'a', 'mu_c', 'mu_r', 'b'}
    ...
    def trial(self, 
        ...
        # Tau depends on the actual time to peak (fixed) and the gamma shape:
        tau = self.time_to_peak/(a-1)
    ...
    def experiment(
        ...
        a : float
            Gamma shape of the automatic activation. `tau` is derived
            internally from `self.time_to_peak` and `a` (see `trial()`), so
            the peak of automatic activation stays fixed at `time_to_peak`
            regardless of `a`.
        mu_c : float
        ...
        data[:obs_per_condition] = self.trial(
            A=A, a=a, mu_c=mu_c, b=b, t=t, noise=noise[:obs_per_condition], non_decision_ts=non_decision_ts[:obs_per_condition], rng=rng
        )
