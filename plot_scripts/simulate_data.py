...
network_names = ['dmc_optimized_winsim_priors_sdr_fixed_200_795737',
                 'dmc_optimized_winsim_priors_sdr_estimated_200_795738' ]

priors = ['initial', 'initial']
sdr = ['fixed', 'estimated']
...
for name in network_names:
    ...
    model_specs_path = parent_dir + '/model_specs/model_specs_' + name + '.pickle'

    with open(model_specs_path, 'rb') as file:
        model_specs = pickle.load(file)

    simulator, adapter, inference_net, summary_net, workflow = dmc_helpers.load_model_specs(model_specs, name)
    approximator = keras.saving.load_model(parent_dir + "/data/training_checkpoints/" + name + '.keras')
...
for sim_idx in range(0, num_sims):
    for i, appr in enumerate(approximators):
        ...
        df_samples['priors'] = priors[i]
        df_samples['sdr'] = sdr[i]
