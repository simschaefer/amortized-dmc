...
data_emp = empirical_data[empirical_data['participant'].isin(train_idx)]
...
delta_data_emp = (
    ...
    .assign(mean_qu=lambda df: (0.5*df['incongruent'] + 0.5*df['congruent']))
)

delta_data_emp['quantile'].unique()
...
delta_data_emp['mean_qu_bins'] = pd.cut(delta_data_emp["mean_qu"], bins=n_delta_bins, include_lowest=True, right=True)
...
# Average across participants *per quantile bin*
delta_bins_emp = (
    delta_data_emp
...
sns.lineplot(delta_bins_emp, marker="o",linestyle='--', linewidth=0.5,  x='mean_qu', y='delta', legend=False,  color='black')
...
sns.lineplot(delta_bins_emp,linewidth=0.5,  x='bin_mid', y='delta', legend=False, ax=axes[2], color='black')
...
#fig.suptitle(model + spacing + data_split, fontsize=14)
