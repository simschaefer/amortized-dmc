...
for spacing, spacing_num in zip(['narrow', 'wide'], [1,0]):
    for part, ax in zip(parts, axes):
        ...
        part_data_samples_fixed = part_data_samples_fixed[part_data_samples_fixed["spacing"] == spacing]

        # filter empirical data for given participant and narrow spacing
        part_data = empirical_data[empirical_data["participant"] == part]

        part_data = part_data[part_data["spacing_num"] == spacing_num]
        ...
        data_resimulated_fixed['model'] = network_name_fixed

        data_resimulated_fixed['spacing'] = spacing

        lst_data.append(data_resimulated_fixed)
        ...
        part_data_samples_estimated = part_data_samples_estimated[part_data_samples_estimated["spacing"] == spacing]
        ...
        data_resimulated_estimated['model'] = network_name_estimated

        data_resimulated_estimated['spacing'] = spacing

        lst_data.append(data_resimulated_estimated)
